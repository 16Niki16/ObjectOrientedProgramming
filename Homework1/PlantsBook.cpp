...
PlantsBook::PlantsBook(PlantsBook&& obj){
...
PlantsBook& PlantsBook::operator=(PlantsBook&& obj)
{
	if (this != &obj) {
		this->erase();
		livingPlace = obj.livingPlace;
...
bool PlantsBook::checkFileExist(char* filename)
{
	std::ifstream iFile;
	iFile.open(filename);
	if(!iFile)
...
//operators >> & <<
std::ostream& operator<<(std::ostream& out, const PlantsBook& obj) {
	out << obj.nameOfPlant << " " << obj.livingPlace << " " << obj.days<<std::endl;
	return out;
}
std::istream& operator>>(std::istream& in, PlantsBook& obj) {
	return in;
=======
#include "PlantsBook.h"
#include <fstream>
