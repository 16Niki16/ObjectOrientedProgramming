...
{
	typePlant = obj.typePlant;
	plantLP = obj.plantLP;
	daysWatering = obj.daysWatering;
	obj.daysWatering = nullptr;
	obj.plantLP = nullptr;
	obj.typePlant = nullptr;
}
...
void Plant::ExtractInfo(char* filename){
	...
	if (isSaved) {
		...
		std::cout<<this->typePlant<<" "<< this->plantLP << " " << this->daysWatering;
	}
	iFile.close();
	}
...
std::ostream& operator<<(std::ostream& out, const Plant& obj){
	out << obj.typePlant << " " << obj.plantLP << " " << obj.daysWatering<<std::endl;
	return out;
}
std::istream& operator>>(std::istream& in, Plant& obj) {
	in.getline(obj.typePlant,256);
	in.getline(obj.plantLP, 256);
	in.getline(obj.daysWatering, 256);
	return in;
}
=======
#include "Plant.h"
